...
/**
 * Simplification mobile par regroupement spatial des sommets.
 *
 * La V4.5 supprimait des triangles isolés à intervalles réguliers, ce qui
 * créait des trous. La V4.6 soude d'abord les sommets voisins, reconstruit les
 * triangles valides et ne recourt à un échantillonnage final qu'en dernier
 * secours pour respecter une limite stricte.
 */
public final class MobileMeshOptimizer {
    private static final int[] GRID_RESOLUTIONS = {
            224, 192, 160, 144, 128, 112, 96, 80, 72, 64,
            56, 48, 40, 36, 32, 28, 24, 20, 18, 16,
    ...
    public static MeshData simplify(MeshData source, int targetTriangles) {
        ...
        int target = Math.max(1, Math.min(totalTriangles, targetTriangles));
        if (totalTriangles <= target) {
            return source;
        }

        MeshData bestUnderBudget = null;
        MeshData closestAboveBudget = source;
        for (int resolution : GRID_RESOLUTIONS) {
            ...
            } else if (triangles < closestAboveBudget.getTriangleCount()) {
    ...
    private static MeshData sampleTriangles(MeshData source, int target) {
        ...
        for (int sample = 0; sample < requested; sample++) {
            ...
            int[] sourceTriangle = {
                    ...
                    sourceIndices[sourceOffset + 1],
                    sourceIndices[sourceOffset + 2]
            };
            boolean valid = true;
            for (int vertex : sourceTriangle) {
                if (!validVertex(vertex, sourceVertexCount)) {
                    valid = false;
                    break;
                }
            }
            if (!valid
                    || sourceTriangle[0] == sourceTriangle[1]
                    || sourceTriangle[1] == sourceTriangle[2]
                    || sourceTriangle[0] == sourceTriangle[2]) {
                continue;
            }
            for (int sourceVertex : sourceTriangle) {
    ...
    private static float triangleAreaSquared(
            Cluster a,
            Cluster b,
            Cluster c
    ) {
